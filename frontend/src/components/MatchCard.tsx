  ...
  return (
    ...
    >
      ...
      `}>
        ...
        {!isLocked && match.isKnockout && enabledBets.winnerOnly && predictionMode !== 'winner' && (
          <div onClick={() => setPredictionMode('winner')}
            className="mt-3 flex items-center gap-2 px-3 py-2 rounded-xl bg-white/5 hover:bg-white/10 cursor-pointer transition-all"
          >
        ...
        {!isLocked && match.isKnockout && enabledBets.winnerOnly && predictionMode === 'winner' && (
          <div className="mt-3 pt-3 border-t border-white/10">
            <div className="flex items-center gap-2 mb-2">
              ...
              <button onClick={() => { setPredictionMode('score'); setWinner(null); }} className="ml-auto text-xs text-white/40 hover:text-white/70">Usar marcador</button>
            </div>
            <div className="flex gap-2">
              {[
                ...
                <button key={opt.value} onClick={() => {
                  ...
                  setWinner(newWinner);
                  if (newWinner) { setHomeScore(''); setAwayScore(''); }
                }}
